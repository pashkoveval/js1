let app = {
    ...
    insertPawns() {
        ...
        for (let i = 0; i < whitePawnsRow.length; i++) {
            ...
            whitePawnsRow[i].innerHTML = this.getFigure(figure, 'whight');
        }
        ...
        for (let i = 0; i < blackPawnsRow.length; i++) {
            ...
            blackPawnsRow[i].innerHTML = this.getFigure(figure, 'black');
        }
    },
    ...
    insertRowsNumbers() {
        ...
        for (let i = 0; i < trs.length; i++) {
            ...
            td.innerText = this.config.rows[0];
            // 7.4 получившийся тег td вставляем в текущую строку (тег tr)
            trs[0].insertAdjacentElement("afterbegin", td);
        }
    },
    ...
    insertColsChars() {
        ...
        for (let i = 0; i < this.config.cols.length; i++) {
            ...
            tr.innerHTML += `<td>${this.config.cols[0]}</td>`;
        }
...
// НЕФИГА НЕ ПОНЯЛ КАК Я ЭТО СДЕЛАЛ НО Я ВСЕ ТАКИ ВЫВЕЛ ИХ НА ДОСКУ. ОЧЕНЬ ТУГО ДОГОНЯЮ ПОСЛЕ ПРАЗДНИКОВ БУДУ РАЗБЕРАТЬСЯ.